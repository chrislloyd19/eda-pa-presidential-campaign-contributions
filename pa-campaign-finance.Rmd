...
ggplot(subset(contributions, contributions$cand_nm == 'Clinton, Hillary Rodham' | contributions$cand_nm == 'Trump, Donald J.'), aes(contribution_date, color=cand_nm)) +
  ...
  xlab("date") +
  ylab("# contributions received") +
  ggtitle("Contribution frequency by date for Hillary Clinton and Donald Trump")
```
...
ggplot(subset(contbr_by_occupation_party, contbr_by_occupation_party$contbr_occupation == 'PHYSICIAN' | contbr_by_occupation_party$contbr_occupation == 'ATTORNEY' | contbr_by_occupation_party$contbr_occupation == 'PROFESSOR' | contbr_by_occupation_party$contbr_occupation == 'TEACHER' | contbr_by_occupation_party$contbr_occupation == 'RETIRED' | contbr_by_occupation_party$contbr_occupation == 'PRESIDENT'), aes(x=contbr_occupation, y=total, color=party)) +
  ...
  ggtitle("Political contribution amounts by most contributing professions and retirees to each party during 2016 presidential campaign")
```

## Plot 3 - Boxplot of $ amounts per contribution for top candidates

This boxplot shows information regarding the $ amount per contribution for the top three candidates: Clinton, Sanders, and Trump.

```{r}
ggplot(subset(contributions, contributions$cand_nm == 'Clinton, Hillary Rodham' | contributions$cand_nm == 'Sanders, Bernard' | contributions$cand_nm == 'Trump, Donald J.'), aes(x=cand_nm, y=contb_receipt_amt)) +
  geom_boxplot() +
  coord_cartesian(ylim = c(1, 2700)) +
  scale_y_log10() +
  xlab("candidate") +
  ylab("contribution amount in dollars") +
  ggtitle("Boxplot of $ per contribution for top 3 candidates in terms of contribution amount")
```


# Reflection

This data has only one quantative variable and a bunch of qualitative variables. So naturally most insights involve the amount of $ contributed to certain groups. The frequency of contributions from donors is a way to gauge enthusiasm.

I unfortunately did not get to analysing donations from certain locations in the state. It must be possible to produce some sort of map, maybe a heatmap, of contributions by zip code. This would be a great visual of enthusiasm for either parties or candidates in various regions of the state. If I were to further this investigation, I would certainly add this.
